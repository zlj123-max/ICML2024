...
class ManyAgentAntEnv(MujocoEnv, utils.EzPickle):
    def __init__(self, **kwargs):
        ...
        self._generate_asset(n_segs=self.n_segs, asset_path=asset_path)
    ...
    #     print("abs(y_walldist)",abs(y_walldist))
    ...
    #     #     print("joint_addr",joint_addr)                                  # zlj
    ...
    def step(self, a):
        xposbefore = [0 for j in range(self.n_segs)]
        xposafter = [0 for j in range(self.n_segs)]
        yposafter = [0 for j in range(self.n_segs)]
        z_rot = [0 for j in range(self.n_segs)]
        obj_cost = 0 
        for i in range(self.n_segs):
            xposbefore[i] = self.get_body_com(str("torso_" + str(i)))[0]
        self.do_simulation(a, self.frame_skip)
        ...
        for i in range(self.n_segs):
            xposafter[i] = self.get_body_com(str("torso_" + str(i)))[0]
        forward_reward = (np.mean(xposafter) - np.mean(xposbefore))/self.dt
        ctrl_cost = .5 * np.square(a).sum()
        contact_cost = 0.5 * 1e-3 * np.sum(
            np.square(np.clip(self.sim.data.cfrc_ext, -1, 1)))
        survive_reward = 1.0
        ...
        ### ADDED safety stuff
        for i in range(self.n_segs):
            yposafter[i] = self.get_body_com(str("torso_" + str(i)))[1]
        ywall = np.array([-4.5, 4.5])
        for i in range(self.n_segs):
            if xposafter[i] < 20:
                y_walldist = yposafter[i] - xposafter[i] * np.tan(30/360*2*np.pi) + ywall
            elif xposafter[i]>20 and xposafter[i]<60:
                y_walldist = yposafter[i] + (xposafter[i]-40)*np.tan(30/360*2*np.pi) - ywall
            elif xposafter[i]>60 and xposafter[i]<100:
                y_walldist = yposafter[i] - (xposafter[i]-80)*np.tan(30/360*2*np.pi) + ywall
            else:
                y_walldist = yposafter[i] - 20*np.tan(30/360*2*np.pi) + ywall
            obj_cost_i = (abs(y_walldist) < 1.8).any() * 1.0
            obj_cost += obj_cost_i
        obj_cost = obj_cost/self.n_segs
        reward = forward_reward - ctrl_cost - contact_cost + survive_reward

        #### ADDED
        for i in range(self.n_segs):
            body_quat = self.data.get_body_xquat(str("torso_" + str(i)))
            z_rot[i] = 1-2*(body_quat[1]**2+body_quat[2]**2)  ### normally xx-rotation, not sure what axes mujoco uses
        state = self.state_vector()
        notdone = np.isfinite(state).all() \
            and state[2] >= 0.2 and state[2] <= 1.0\
            and min(z_rot)>=-0.7 #ADDED
    ...
    def _get_obs(self):
        ...
        return np.concatenate([
            self.sim.data.qpos.flat[3: 3 + 6*self.n_segs], # size = 3
            self.sim.data.qvel.flat, # size = 6
            self.sim.data.qpos.flat[3 + 6*self.n_segs:], # size = 3
            [x/5],
            [y_off],
        ]) 
