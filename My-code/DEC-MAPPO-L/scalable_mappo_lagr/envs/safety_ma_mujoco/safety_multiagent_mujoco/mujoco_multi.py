...
class MujocoMulti(MultiAgentEnv):
    ...
    def __init__(self, batch_size=None, **kwargs):
        ...
        print("all_args.agent_obsk", self.agent_obsk)
    ...
    def get_obs_agent(self, agent_id):
        if self.agent_obsk is not None:
            ...
            obs_radi = self.agent_obsk//2 + 1
            if self.scenario in ["manyagent_ant", "Ant-v2"]:
                for a in range(obs_radi):
                    if agent_id - obs_radi <= 0 :
                        return np.concatenate([
                            obs[: 6*self.agent_obsk],
                            obs[6*self.n_agents: 6*(self.n_agents+self.agent_obsk)],
                            obs[12*self.n_agents: ],
                        ]) 
                    elif agent_id - obs_radi > 0 and agent_id + obs_radi < self.n_agents :
                        return np.concatenate([
                            obs[6*(agent_id-obs_radi): 6*(agent_id-obs_radi+self.agent_obsk)],
                            obs[6*(agent_id-obs_radi+self.n_agents): 6*(agent_id-obs_radi+self.agent_obsk+self.n_agents)],
                            obs[12*self.n_agents: ],
                        ])  
                    elif agent_id + obs_radi >= self.n_agents :
                        return np.concatenate([
                            obs[6*(self.n_agents-self.agent_obsk): 6*self.n_agents],
                            obs[12*self.n_agents-6*self.agent_obsk: 12*self.n_agents],
                            obs[12*self.n_agents: ],
                        ]) 
            elif self.scenario in ["coupled_half_cheetah"]:
                for a in range(obs_radi):
                    if agent_id - obs_radi <= 0 :
                        return np.concatenate([
                            obs[: self.agent_obsk],
                            obs[self.n_agents: self.n_agents+self.agent_obsk],
                            obs[2*self.n_agents: ],
                        ]) 
                    elif agent_id - obs_radi > 0 and agent_id + obs_radi < self.n_agents :
                        return np.concatenate([
                            obs[agent_id-obs_radi: agent_id-obs_radi+self.agent_obsk],
                            obs[agent_id-obs_radi+self.n_agents: agent_id-obs_radi+self.agent_obsk+self.n_agents],
                            obs[2*self.n_agents: ],
                        ])  
                    elif agent_id + obs_radi >= self.n_agents :
                        return np.concatenate([
                            obs[self.n_agents-self.agent_obsk: self.n_agents],
                            obs[2*self.n_agents-self.agent_obsk: 2*self.n_agents],
                            obs[2*self.n_agents: ],
                        ])
            elif self.scenario in ["HalfCheetah-v2"]:
                for a in range(obs_radi):
                ...
                for a in range(obs_radi):
                    if agent_id - obs_radi <= 0 :
                        return np.concatenate([
                            obs[: 3*self.agent_obsk],
                            obs[3*self.n_agents: 3*(self.n_agents+self.agent_obsk)],
                            obs[6*self.n_agents: ],
                        ]) 
                    elif agent_id - obs_radi > 0 and agent_id + obs_radi < self.n_agents :
                        return np.concatenate([
                            obs[3*(agent_id-obs_radi): 3*(agent_id-obs_radi+self.agent_obsk)],
                            obs[3*(agent_id-obs_radi+self.n_agents): 3*(agent_id-obs_radi+self.agent_obsk+self.n_agents)],
                            obs[6*self.n_agents: ],
                        ])  
                    elif agent_id + obs_radi >= self.n_agents :
                        return np.concatenate([
                            obs[3*(self.n_agents-self.agent_obsk): 3*self.n_agents],
                            obs[6*self.n_agents-3*self.agent_obsk: 6*self.n_agents],
                            obs[6*self.n_agents: ],
                        ])
            ...
            return build_obs(self.env,
                             ...
                             vec_len=getattr(self, "obs_size", None))                  # zlj                        
