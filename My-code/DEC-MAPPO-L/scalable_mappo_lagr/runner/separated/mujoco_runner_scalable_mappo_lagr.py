...
import torch
from scalable_mappo_lagr.runner.separated.base_runner_scalable_mappo_lagr import Runner
...
class MujocoRunner(Runner):
    ...
    def warmup(self):
        ...
        #     share_obs = obs
        for agent_id in range(self.num_agents):
            # print("self.buffer[agent_id].share_obs[0]", self.buffer[agent_id].share_obs[0])
            # print("share_obs[:, agent_id]", share_obs[:, agent_id])                           # zlj
    ...
    def collect(self, step):
        ...
        for agent_id in range(self.num_agents):
            ...
            # print("self.buffer[agent_id].share_obs[step]", self.buffer[agent_id].share_obs[step])
            # print("self.buffer[agent_id].obs[step]", self.buffer[agent_id].obs[step])                     # zlj
    ...
    def insert(self, data):
        ...
        # print("share_obs.shape", share_obs.shape)      #  zlj
